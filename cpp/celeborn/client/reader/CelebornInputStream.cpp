...
std::unique_ptr<ReadOnlyByteBuffer> CelebornInputStream::getNextChunk() {
...
void CelebornInputStream::verifyChunk(
    const std::unique_ptr<ReadOnlyByteBuffer>& chunk) {
...
std::shared_ptr<PartitionReader> CelebornInputStream::createReader(
  ...
  switch (location.storageInfo->type) {
    case StorageInfo::HDD:
    case StorageInfo::SSD: {
    ...
    default:
      ...
      CELEBORN_FAIL(
          "unsupported StorageInfo type " +
          std::to_string(location.storageInfo->type));
  }
}

std::shared_ptr<const PartitionLocation>
