...
#include "celeborn/utils/ProtoUtils.h"
...
TEST(ControlMessagesTest, getReducerFileGroup) {
  ...
  auto pbGetReducerFileGroup = parseProto<PbGetReducerFileGroup>(
...
TEST(ControlMessagesTest, openStream) {
  ...
  auto payload = transportMessage.payload();
  auto pbOpenStream = parseProto<PbOpenStream>(
      reinterpret_cast<const uint8_t*>(payload.c_str()), payload.size());
...
TEST(ControlMessagesTest, streamChunkSlice) {
  ...
  auto writeBuffer = ByteBuffer::createWriteOnly(20);
  ...
  auto readBuffer = ByteBuffer::toReadOnly(std::move(writeBuffer));
...
TEST(ControlMessagesTest, chunkFetchRequest) {
  ...
  auto payload = transportMessage.payload();
  auto pbChunkFetchRequest = parseProto<PbChunkFetchRequest>(
      reinterpret_cast<const uint8_t*>(payload.c_str()), payload.size());
...
TEST(ControlMessagesTest, bufferStreamEnd) {
  ...
  auto payload = transportMessage.payload();
  auto pb = parseProto<PbBufferStreamEnd>(
      reinterpret_cast<const uint8_t*>(payload.c_str()), payload.size());
