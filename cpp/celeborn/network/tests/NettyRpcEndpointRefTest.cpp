 ...
 public:
  MockTransportClient()
      : TransportClient(nullptr, nullptr, MS(100)),
        response_(RpcResponse(0, ReadOnlyByteBuffer::createEmptyBuffer())),
        request_(RpcRequest(0, ReadOnlyByteBuffer::createEmptyBuffer())) {}
...
void readUTF(ReadOnlyByteBuffer& buffer, std::string& host) {
  int size = buffer.read<short>();
  host = buffer.readToString(size);
}

void readRpcAddress(ReadOnlyByteBuffer& buffer, std::string& host, int& port) {
...
TEST(NettyRpcEndpointRefTest, askSyncGetReducerFileGroup) {
  ...
  auto writeBuffer = ByteBuffer::createWriteOnly(sizeof(uint8_t));
  writeBuffer->write<uint8_t>(NettyRpcEndpointRef::kNativeTransportMessageFlag);
  auto flagBody = ByteBuffer::toReadOnly(std::move(writeBuffer));
  auto concatBody = ByteBuffer::concat(*flagBody, *msgBody);

  auto rpcResponse =
      std::make_unique<RpcResponse>(1000, std::move(concatBody));
  mockedClient->setResponse(*rpcResponse);

  GetReducerFileGroup request{1001};
